...
const PositionModal = ({ onClose }) => {
  ...
  const handleAdd = async (e) => {
    ...
    if (!name) return;
    if (positions.some(p => p.name.toLowerCase() === name.toLowerCase())) {
      setError('Position already exists.'); return;
    }
  ...
  const handleUpdate = async (e) => {
    ...
    if (!name) return;
    if (positions.some(p => p.name.toLowerCase() === name.toLowerCase() && p.id !== editingId)) {
      setError('Position already exists.'); return;
    }
    await updatePosition(editingId, name);
    setEditingId(null);
    setEditingName('');
    setError('');
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this position? Users assigned to it will be unassigned.')) return;
      ...
      <div className={styles.modal} onClick={e => e.stopPropagation()} style={{ maxWidth: '480px' }}>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.section}>
            <h3 style={{ marginBottom: '12px' }}>Add Position</h3>
          ...
          <div className={styles.section}>
            <h3 style={{ marginBottom: '12px' }}>
              Position List <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: '0.85rem' }}>({positions.length})</span>
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {positions.length === 0 && (
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>No positions yet.</p>
              )}
              {positions.map(pos => (
                <div key={pos.id} style={rowStyle}>
                  {editingId === pos.id ? (
                    ...
                    <>
                      <span style={{ flex: 1, fontSize: '0.88rem', color: 'var(--text-main)' }}>{pos.name}</span>
                      <button style={iconBtn()} title="Edit" onClick={() => startEdit(pos)}>
                        <Pencil size={15} />
                      </button>
                      <button style={iconBtn(true)} title="Delete" onClick={() => handleDelete(pos.id)}>
                        <Trash2 size={15} />
                      </button>
                    </>
                  )}
